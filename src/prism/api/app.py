...
from prism.application.errors import ApplicationError, EventStreamOverflowError
...
def create_app(service: ApplicationService) -> FastAPI:
    ...
    app = FastAPI(
        title="Prism API",
        version=__version__,
        description="Local versioned control API for one Prism project.",
    )
    app.state.prism_service = service
