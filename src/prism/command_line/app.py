...
from prism.api.server import _is_loopback_host, run_server
...
def version(
    ...
    """Print the installed Prism version."""
...
def demo(
    ...
    def action() -> CommandResult:
        ...
        if destination.suffix.casefold() != ".prism-work":
            raise CliFailure(
                CliExit.USAGE,
                ApiIssue(
                    code="invalid_project_path",
                    path="/path",
                    message="Demo path must end with .prism-work",
                ),
            )
        ...
        if not _is_loopback_host(host):
            raise CliFailure(
                ...
                ApiIssue(
                    ...
                    path="/host",
                    message="Prism may bind only to a loopback address",
                ),
                project=context,
            )
...
def project_init(
    path: Path = typer.Argument(..., help="Destination .prism or .prism-work project."),
    ...
    def action() -> CommandResult:
        ...
        if suffix not in {".prism", ".prism-work"}:
            raise CliFailure(
                CliExit.USAGE,
                ApiIssue(
                    code="invalid_project_path",
                    path="/path",
                    message="Project path must end with .prism or .prism-work",
                ),
            )
...
def project_migrate(
    ...
    def action() -> CommandResult:
        ...
        if resolved.suffix.casefold() != ".prism":
            raise CliFailure(
                CliExit.USAGE,
                ApiIssue(
                    code="invalid_project_path",
                    path="/path",
                    message="Only portable .prism archives can be migrated",
                ),
            )
    ...
    def action() -> CommandResult:
        ...
        with connected_project(project, url) as service:
            if idempotency_key is None:
                ...
                token = f"prism-cli-audio-import:{idempotency_key}:{source_sha256}"
...
def _portable_project(path: Path) -> tuple[Project, Path]:
    resolved = path.resolve(strict=True)
    if resolved.suffix.casefold() != ".prism" or not resolved.is_file():
        raise CliFailure(
            CliExit.VALIDATION,
            ApiIssue(
                code="portable_archive_required",
                path="/project",
                message="--portable requires an existing .prism archive",
            ),
        )
