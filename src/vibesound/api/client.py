...
class VibeSoundClient:
    ...
    def events(self, project_id: UUID) -> VibeSoundEventStream:
        return VibeSoundEventStream(self._base_url, project_id, timeout=self._timeout)
        ...
        if response.is_error and not allow_error_envelope:
            ...
            if not issues:
                issues = [
                    ApiIssue(
                        code="http_error",
                        message=f"VibeSound returned HTTP {response.status_code}",
                    )
                ]
