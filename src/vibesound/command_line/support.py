...
from vibesound.application import ApiIssue, BackgroundJob, ReadinessResult, TransactionRequest
...
def run_command(
    ...
    *,
    ...
    except VibeSoundClientError as error:
...
def emit_stream_failure(command: str, as_json: bool, error: BaseException) -> None:
    ...
    if isinstance(error, CliFailure):
        failure = error
    elif isinstance(error, VibeSoundClientError):
        failure = CliFailure(_client_exit(error), error.issues)
    ...
    if not _is_loopback(parsed.hostname):
        raise CliFailure(
            ...
            ApiIssue(
                ...
                path="/url",
                message="VibeSound CLI connections are restricted to loopback hosts",
            ),
        )
...
def read_local_project(path: Path | str) -> LocalProject:
    ...
    if resolved.is_dir():
        if resolved.suffix.casefold() != ".vibesound-work":
            raise CliFailure(
                CliExit.VALIDATION,
                ApiIssue(
                    code="invalid_project_path",
                    path="/project",
                    message="Working project directories must end with .vibesound-work",
                ),
            )
        project = _read_working_manifest(resolved)
    elif resolved.suffix.casefold() == ".vibesound":
        ...
        raise CliFailure(
            ...
            ApiIssue(
                code="invalid_project_path",
                path="/project",
                message="Project must be a .vibesound archive or .vibesound-work directory",
            ),
        )
...
def connected_project(
    ...
    *,
    ...
    try:
        with VibeSoundClient(normalized_url, timeout=timeout) as client:
...
def list_entities(client: VibeSoundClient, project_id: UUID, entity_type: str) -> list[Any]:
...
def _client_exit(error: VibeSoundClientError) -> CliExit:
